...
if __name__ == "__main__":
    ...
    for i in range(blocks):
        if i == 0:
            ...
            start_time_assy += np.floor(st.chi2.rvs(df=1.53, loc=-0, scale=0.2069))

        process_time_assy, process_time_oft, process_time_pnt = 0.0, 0.0, 0.0
        while process_time_assy <= 0:
            process_time_assy = st.exponnorm.rvs(K=5.13, loc=5.50, scale=1.86)
        while process_time_oft <= 0:
            process_time_oft = st.gamma.rvs(a=0.98, loc=1.00, scale=14.1)
        while process_time_pnt <= 0:
            process_time_pnt = st.exponnorm.rvs(K=1.25, loc=8.66, scale=2.96)
