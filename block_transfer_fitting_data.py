...
if __name__ == "__main__":
    ...
    for i in range(blocks):
        if i == 0:
            ...
            start_time_assy += np.floor(st.chi2.rvs(df=1.53, loc=-0, scale=0.2064))
    ...
    # 평균 WIP
    print("average WIP of Assembly : {}".format(np.mean(Monitor1.WIP)))
    print("average WIP of Outfitting : {}".format(np.mean(Monitor2.WIP)))
    print("average WIP of Painting : {}".format(np.mean(Monitor3.WIP)))

    # 가동률
    total_assy = (Assembly.process_finish - Assembly.process_start) * m_assy
    total_oft = (Outfitting.process_finish - Outfitting.process_start) * m_oft
    total_pnt = (Painting.process_finish - Painting.process_start) * m_pnt
